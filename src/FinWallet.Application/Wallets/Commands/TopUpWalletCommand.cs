...
public sealed record TopUpWalletCommand(Guid WalletId, decimal Amount, string Reference) : IRequest<TransactionDto>;
...
public sealed class TopUpWalletCommandHandler : IRequestHandler<TopUpWalletCommand, TransactionDto>
{
    ...
    public async Task<TransactionDto> Handle(TopUpWalletCommand request, CancellationToken cancellationToken)
    {
        ...
        var tx = wallet.ApplyCredit(request.Amount, request.Reference, TransactionType.TopUp);
